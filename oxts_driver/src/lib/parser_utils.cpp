...
#include <oxts_driver/parser_utils.hpp>

namespace lib_oxts {

namespace NComParser {
...
tf2::Quaternion getBodyRPY(const NComRxC *nrx) {
  auto rpyVehENU =
      lib_oxts::NComParser::getVehRPY(nrx); // Orientation of the vehicle (ENU frame)
  auto rpyBodENU = tf2::Quaternion(); // Orientation of the body (ENU frame)
  auto vat = getVat(nrx); // Body to vehicle frame rotation
...
Lrf getNcomLrf(const NComRxC *nrx) {
  ...
  // return (nrx->mRefHeading - 90) * NAV_CONST::DEG2RADS;
}
...
nav_msgs::msg::Odometry odometry(const NComRxC *nrx,
  ...
  // ENU - tangent
  auto rpyBodENU = lib_oxts::NComParser::getBodyRPY(nrx);
  ...
  auto twist_stamped = lib_oxts::NComParser::velocity(nrx, head);
